...
The HealthCheck::Diagnostic module provides a base for writing your
...
The implementation for that check might look something like this:
    ...
    sub desired_state {
        ...
        my $too_cold      = $inside < $self->{desired_min};
        my $too_hot       = $inside > $self->{desired_max};
        my $cooler_inside = $inside < $outside;
        my $warmer_inside = $inside > $outside;

        if (   ( $too_cold and $cooler_inside )
            or ( $too_hot  and $warmer_inside ) )
        {
            return 'open';
        }
